...
class YoudoParser(BaseParser):
    ...
    async def get_tasks(self, session, url: str, seen_checker=None) -> list:
        tasks = await self._fetch_via_post(session, seen_checker=seen_checker)
        if tasks:
    ...
    async def _fetch_via_post(self, session, seen_checker=None) -> list:
        all_tasks = []
        page = 1
        while True:
            ...
            # Задания отсортированы новые первыми. Если вся страница уже известна —
            # дальше будут только ещё более старые, смысла тянуть нет.
            if seen_checker:
                page_urls = [t["url"] for t in tasks if t.get("url")]
                if page_urls and all(seen_checker(u) for u in page_urls):
                    logger.info(f"[YouDo] Стр. {page} целиком известна — досрочный стоп.")
                    break
